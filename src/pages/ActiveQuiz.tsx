        ...
        <section className="w-full max-w-5xl mx-auto rounded-3xl bg-[rgba(12,18,30,0.85)] backdrop-blur-xl border border-white/10 shadow-[0_30px_80px_rgba(0,0,0,0.48),0_10px_30px_rgba(10,16,26,0.35)] flex flex-col overflow-hidden text-slate-200">
          ...
          <div className="p-6 sm:p-8 md:p-10 flex flex-col gap-8 flex-1 justify-between">
            <div className="space-y-4">
              <div className="text-xs font-mono tracking-widest text-[#a2d4f8] uppercase font-medium">
                Concept Drill
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {currentQuestion.options.map((opt, idx) => {
                ...
                let textClass =
                  "font-serif text-[15px] sm:text-base md:text-lg overflow-hidden break-words ";

                if (isAnswered) {
                  if (isSelected && isOptionCorrect) {
                    containerClass +=
                      "border-emerald-400/80 bg-emerald-500/10 shadow-[0_0_15px_rgba(52,211,153,0.15)] ring-1 ring-emerald-400/40 pointer-events-none";
                    letterClass +=
                      "bg-emerald-400 text-emerald-950 shadow-sm shadow-emerald-400/50";
                    textClass += "text-white font-medium";
                  } else if (isSelected && !isOptionCorrect) {
                    containerClass +=
                      "border-rose-400/80 bg-rose-500/10 shadow-[0_0_15px_rgba(251,113,133,0.15)] ring-1 ring-rose-400/40 pointer-events-none";
                    letterClass +=
                      "bg-rose-400 text-rose-950 shadow-sm shadow-rose-400/50";
                    textClass +=
                      "text-white font-medium line-through decoration-rose-500/50 opacity-70";
                  } else if (isOptionCorrect) {
                    containerClass +=
                      "border-emerald-400/50 bg-white/[0.02] pointer-events-none";
                    letterClass +=
                      "bg-emerald-400/20 text-emerald-300 border border-emerald-400/30";
                    textClass += "text-emerald-300";
                  } else {
                    containerClass +=
                      "border-white/5 bg-white/[0.01] opacity-50 pointer-events-none";
                    letterClass +=
            ...
            {showExplanation && currentQuestion.explanation && (
              <div className="p-4 rounded-xl bg-white/[0.03] border border-white/10 flex items-start gap-3 text-sm text-slate-300 mt-2">
                <svg
                  className="w-5 h-5 text-[#a2d4f8] shrink-0 mt-0.5"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
                ...
                <div className="leading-relaxed">
                  <strong className="text-white font-medium mr-2">
                    Insight:
                  </strong>
                ...
                <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-white/[0.04] border border-white/10">
                  {questions.map((q, i) => {
                    ...
                    if (i === currentQuestionIndex) {
                      dotClass +=
                        "w-4 bg-[#a2d4f8] shadow-[0_0_10px_rgba(162,212,248,0.5)]";
