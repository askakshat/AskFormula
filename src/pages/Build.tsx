    ...
    <div className="min-h-screen overflow-x-hidden bg-[#0b0e15] text-[#e1e2ec] font-sans flex flex-col">
      <header className="bg-surface-container-lowest dark:bg-[#0b0e15] fixed top-0 w-full z-50 border-b border-[#32353c] transition-colors duration-200">
        <div className="flex justify-between items-center px-6 md:px-12 py-3 max-w-[1200px] mx-auto h-16">
          <div className="flex items-center gap-3 cursor-pointer active:opacity-80" onClick={() => window.location.href = '/'}>
            <span className="font-headline-md text-xl font-bold text-[#aec6ff]">AskFormula</span>
          </div>
          <div className="flex items-center">
             <span className="font-body-md text-[14px] text-[#e1e2ec]/70">Build Flow</span>
          </div>
        </div>
      </header>
        ...
        <div className="w-full max-w-[1200px] px-6 md:px-12 py-8 flex flex-col md:flex-row gap-12 relative">
          ...
          <aside className="w-full md:w-64 flex-shrink-0">
            <div className="sticky top-32">
              <div className="mb-6">
                <h2 className="text-xl font-semibold text-[#aec6ff]">Configuration</h2>
                <p className="text-[14px] text-[#e1e2ec]/70 mt-1">Select your parameters</p>
              </div>
              <div className="flex flex-col gap-4 relative">
                <div className="absolute left-[11px] top-4 bottom-4 w-px bg-[#32353c] -z-10" />

                {/* Steps */}
                <div className="flex items-center gap-3">
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[12px] z-10 ${exam ? "bg-[#00275d] text-[#aec6ff]" : "bg-[#272a31] border border-[#32353c] text-[#e1e2ec]/50"}`}>1</div>
                  <span className={`text-[13px] ${exam ? "text-[#aec6ff]" : "text-[#e1e2ec]/50"}`}>Board</span>
                </div>
                <div className="flex items-center gap-3">
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[12px] z-10 ${selectedClass ? "bg-[#00275d] text-[#aec6ff]" : "bg-[#272a31] border border-[#32353c] text-[#e1e2ec]/50"}`}>2</div>
                  <span className={`text-[13px] ${selectedClass ? "text-[#aec6ff]" : "text-[#e1e2ec]/50"}`}>Class</span>
                </div>
                <div className="flex items-center gap-3">
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[12px] z-10 ${subject ? "bg-[#00275d] text-[#aec6ff]" : "bg-[#272a31] border border-[#32353c] text-[#e1e2ec]/50"}`}>3</div>
                  <span className={`text-[13px] ${subject ? "text-[#aec6ff]" : "text-[#e1e2ec]/50"}`}>Subject</span>
                </div>
                <div className="flex items-center gap-3">
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[12px] z-10 ${selectedChapters.length > 0 ? "bg-[#00275d] text-[#aec6ff]" : "bg-[#272a31] border border-[#32353c] text-[#e1e2ec]/50"}`}>4</div>
                  <span className={`text-[13px] ${selectedChapters.length > 0 ? "text-[#aec6ff]" : "text-[#e1e2ec]/50"}`}>Chapters</span>
                </div>
              </div>

              {exam && selectedClass && subject && (
                <div className="mt-8 bg-[#272a31]/50 border border-[#aec6ff]/20 rounded-lg p-4 flex flex-col gap-2">
                  <span className="text-[12px] text-[#aec6ff] font-medium uppercase tracking-wider">Current Context</span>
                  <div className="flex flex-col gap-1 mt-2">
                    <p className="text-[14px] text-[#e1e2ec] font-medium">{exam === 'school' ? 'CBSE Board' : exam === 'jee' ? 'JEE Mains' : 'NEET'}</p>
                    <p className="text-[13px] text-[#e1e2ec]/70">Class {selectedClass}</p>
