          ...
          >
            <div className="flex items-center justify-center gap-2">
              <span className="capitalize font-medium">
                {exam === "school" ? "CBSE/State Board" : exam.toUpperCase()}
              </span>
              {isNeet && (
...
const SubjectSelector = ({
  ...
  let subjects = allSubjects
    .filter((s) => s.audience.includes(exam))
    .map((s) => s.subject);
  ...
  if (exam === "jee") {
    ...
    subjects = subjects.filter(
      (s) => !s.startsWith("JEE") && !s.startsWith("NEET"),
    );
  }
      ...
      <div className="flex gap-3 overflow-x-auto pb-2 custom-scrollbar">
        ...
        {subjects.length === 0 && (
          <span className="text-slate-500 text-sm">
            Please select an exam/class first.
          </span>
        )}
      </div>
    </div>
          ...
          (ch: {
            ...
            return (
              <button
                ...
                onClick={() => {
                  if (isSelected)
                    onSelect(selectedIds.filter((id: string) => id !== ch.id));
              ...
              >
                <div
                  className={`mt-0.5 shrink-0 w-4 h-4 rounded border flex items-center justify-center ${isSelected ? "bg-[#61dcb0] border-[#61dcb0]" : "border-slate-500"}`}
                >
...
export default function QuizDashboard() {
  ...
  useEffect(() => {
    const validIds = new Set(
      allSubjects.flatMap((s) => s.chapters.map((c) => c.id)),
    );
      ...
      subject.chapters.forEach((chapter) => {
        if (selectedChapters.includes(chapter.id)) {
          if (!infos.find((i) => i.id === chapter.id)) {
            infos.push({
              id: chapter.id,
              name: chapter.name || chapter.chapterName || "Unknown",
        ...
        <div className="flex-1 w-full flex flex-col gap-6">
          <div>
            ...
            <p className="text-slate-400 text-sm max-w-lg">
              Build your custom quiz by selecting multiple chapters across
              different subjects, classes, or exams. Add them to your practice
              pool to begin.
            </p>
          </div>
              ...
              {selectedChapters.length > 0 ? (
                <>
                  ...
                  <p className="text-slate-400 text-sm">
                    You have selected {selectedChapters.length} chapter
                    {selectedChapters.length === 1 ? "" : "s"}. Our zero-compute
                    engine will generate identification, numerical, and
                    proportionality questions directly from your selected
                    syllabus.
                  </p>
                </>
              ) : (
                <>
                  <h4 className="text-xl font-semibold text-white mb-1">
                    Select chapters above
                  </h4>
                  <p className="text-slate-400 text-sm">
                    To view insights, build your formula sheets, or practice
                    problems, please add chapters to your selection pool.
                  </p>
                </>
              )}
            </div>
        ...
        <div className="w-full md:w-80 shrink-0 flex flex-col gap-4 sticky top-24">
          <div className="bg-[#15171e] rounded-xl border border-[#272a31] overflow-hidden flex flex-col shadow-xl">
            <div className="p-4 border-b border-[#272a31] bg-[#1c1e26] flex items-center justify-between">
              ...
              <span className="bg-[#324565]/30 text-[#d8e2ff] text-xs px-2 py-0.5 rounded-full font-mono">
              ...
              {selectedChaptersInfo.length === 0 ? (
                ...
                <div className="flex flex-col gap-2">
                  {selectedChaptersInfo.map((info) => (
                    <div
                      key={info.id}
                      className="bg-[#11131a] border border-[#272a31] rounded-lg p-3 flex justify-between items-start group"
                    >
                      <div className="flex flex-col">
                        <span className="text-[10px] font-semibold text-[#61dcb0] mb-0.5 uppercase tracking-wider">
                          {info.subject}
                        </span>
                        <span className="text-sm text-slate-300 leading-snug pr-4">
                          {info.name}
                        </span>
                      </div>
                      <button
          ...
          <div className="bg-[#11131a] border border-[#272a31] rounded-xl p-4 text-sm text-slate-400 flex flex-col gap-2 shadow-lg">
            <p className="flex items-start gap-2">
              ...
              <span className="leading-relaxed">
                The engine dynamically generates identification, computation,
                and proportionality questions directly from your selected
                syllabus.
              </span>
            </p>
            ...
            <p className="flex items-start gap-2">
              ...
              <span className="leading-relaxed">
                All generated data relies strictly on the official static
                formulas configured in your sheets. No LLM APIs are used during
                the quiz.
              </span>
            </p>
          </div>
