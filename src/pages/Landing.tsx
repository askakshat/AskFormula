    ...
    <div className="landing-page">
      ...
      <section id="templates" className="templates-section page-section">
        ...
        <div className="template-grid">
          {templates.map(({ href, eyebrow, title, meta, icon: Icon, tone }) => <Link to={href} key={href} className={`template-card template-${tone}`}><div className="template-card-top"><span className="template-icon"><Icon size={19} /></span><ArrowUpRight className="template-arrow" size={19} /></div><div className="template-lines" aria-hidden="true"><span /><span /><span /><span /></div><p className="template-eyebrow">{eyebrow}</p><h3>{title.split("\n").map((line) => <span key={line}>{line}<br /></span>)}</h3><p className="template-meta">{meta}</p></Link>)}
        </div>
      </section>
