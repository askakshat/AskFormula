...
function StoryBoxes() {
  return <section className="story-boxes" style={{ display: "flex", flexDirection: "column", gap: "24px", maxWidth: "1000px", margin: "80px auto", padding: "0 24px" }}>
    <motion.div {...fadeUp(0)} style={{ textAlign: "center", marginBottom: "40px" }}>
      <p className="section-label" style={{ color: "var(--muted-paper)" }}>ONE SPACE / THREE MOMENTS</p>
    </motion.div>
      ...
      return (
        ...
        >
          <div style={{ direction: "ltr" }}>
            <span style={{ color: "rgba(255,255,255,.42)", fontSize: "10px", letterSpacing: ".16em", fontFamily: "ui-monospace,monospace" }}>{item.label}</span>
            <h2 style={{ margin: "18px 0", color: "var(--paper)", fontSize: "clamp(32px,3vw,48px)", fontWeight: 420, letterSpacing: "-.06em", lineHeight: 1.1 }}>
              {item.title.split("\n").map((line, i) => {
                if(i===1) return <span key={line} style={{ fontFamily: "\"Instrument Serif\", serif", fontStyle: "italic", color: "rgba(255,255,255,0.7)" }}>{line}<br /></span>
                return <span key={line}>{line}<br /></span>
              })}
            </h2>
            <p style={{ color: "rgba(255,255,255,.52)", fontSize: "14px", lineHeight: 1.6, maxWidth: "340px" }}>{item.body}</p>
          </div>
          <div style={{ direction: "ltr", position: "relative", height: "360px", background: "linear-gradient(145deg, #111, #000 72%)", borderRadius: "16px", overflow: "hidden", border: "1px solid rgba(255, 255, 255, 0.08)", boxShadow: "0 20px 40px rgba(0,0,0,0.8)" }}>
            ...
            {index === 0 && <div className="library-preview"><div className="library-heading"><span>Formula library</span><small>312 results</small></div><div className="library-search">Search across Physics, Chemistry, Maths, and Biology <span>⌘ K</span></div><div className="library-list"><div><b style={{color: '#fff'}}>01</b><span>Equations of Motion</span><i>Physics</i></div><div><b style={{color: '#fff'}}>02</b><span>Work, Energy & Power</span><i>Physics</i></div><div><b style={{color: '#fff'}}>03</b><span>Quadratic Equations</span><i>Mathematics</i></div><div><b style={{color: '#fff'}}>04</b><span>Current Electricity</span><i>Physics</i></div></div></div>}

            {index === 1 && <div className="builder-preview"><div className="builder-title"><span>Build your reference sheet</span><small>STEP 02 OF 04</small></div><div className="builder-columns"><div className="builder-options"><span className="selected-option" style={{borderColor: "rgba(255,255,255,0.4)", color: "#fff", background: "rgba(255,255,255,0.08)"}}>JEE Main</span><span>CBSE / NCERT</span><span>NEET</span></div><div className="builder-formula"><small style={{color: '#fff'}}>SELECTED CHAPTER</small><strong>Kinematics</strong><div className="builder-progress"><span style={{background: '#fff'}} /></div><p>12 formulas will be added to your sheet.</p></div></div></div>}

            {index === 2 && <div className="sheet-preview" style={{ margin: "20px auto 0", transform: "scale(0.85)", transformOrigin: "top center", background: "transparent" }}><div className="sheet-meta">JEE MAIN · CLASS 11 <span style={{color: '#fff'}}>EXPORT PDF</span></div><div className="sheet-title">Kinematics</div><div className="sheet-equation">v² = u² + 2as</div><div className="sheet-rule" /><div className="sheet-vars"><span><b style={{color: '#fff'}}>v</b> final velocity</span><span><b style={{color: '#fff'}}>u</b> initial velocity</span><span><b style={{color: '#fff'}}>a</b> acceleration</span><span><b style={{color: '#fff'}}>s</b> displacement</span></div><div className="sheet-check"><Check size={13} /> 12 formulas organized</div></div>}
          </div>
        </motion.div>
      );
    })}
